    ...
    if res.data:
      ...
      for fila in res.data:
        for col in ["modelo_gemini", "modelo_claude", "modelo_openai"]:
          if fila.get(col):
            modelos_sql.extend(
                [m.strip() for m in fila[col].split(",") if m.strip()]
            )
...
def generar_pdf_resultado(intento):
  ...
  story.append(
      Paragraph(
          f"<b>Empleado:</b> {intento.get('nombre_empleado')} | <b>Fecha:</b>"
  ...
  if erroneas:
    ...
    for idx_e, err in enumerate(erroneas, 1):
      ...
      story.append(
          Paragraph(
              f"Respuesta registrada: <i>{err.get('opcion_elegida')}</i>",
              err_style,
          )
      )
...
def renderizar_temporizador_realtime(idx):
  ...
  if st.session_state.tiempo_inicio_examen:
    ...
    st.info(
        ...
        f" {tiempo_restante_examen // 60:02d}:{tiempo_restante_examen % 60:02d} minutos"
    )
...
def renderizar_temporizador_revision():
  ...
  if tiempo_revision_restante > 0:
    st.warning(
        ...
        f" **{tiempo_revision_restante // 60:02d}:{tiempo_revision_restante % 60:02d} minutos**."
...
if not st.session_state.autenticado:
  ...
  try:
    res_usuarios = (
        supabase.table("empleados").select("*").eq("activo", True).execute()
    )
...
# ---------------------------------------------------------
    ...
    if not st.session_state.examen_finalizado:
      ...
      for i, p_item in enumerate(st.session_state.preguntas_seleccionadas):
        ...
        with c1:
          ...
          st.caption(
              f"Categoría/Tema: **{p_item.get('subindice', 'General')}** |"
  ...
  # CUESTIONARIO ACTIVO
    ...
    if idx < total_p:
      ...
      with col_info:
        ...
        st.caption(
            "📌 **Subíndice/Categoría:**"
            f" {p_actual.get('subindice', p_actual.get('apartado', 'General'))}"
            f" | Dificultad: **{p_actual.get('dificultad', 'dificil')}**"
        )
      with col_ayuda:
        st.caption(
            "💡 Ayudas disponibles:"
  ...
  # MENÚ PRINCIPAL
    ...
    with tab_examenes:
      ...
      try:
        res_examenes = (
            supabase.table("examenes").select("*").eq("activo", True).execute()
        )
      ...
      if examenes_disponibles:
        ...
        with tab_global:
          ...
          if ya_hecho_global:
            ...
            st.warning(
                f"⚠️ **REALIZADO ESTE MES** — Nota previa: **{info_g['nota']} /"
          ...
          for idx_ex, ex_obj in enumerate(examenes_disponibles):
            ...
            with cols[idx_ex % 3]:
              ...
              if ya_hecho_manual:
                ...
                st.caption(
                    f"⚠️ **Realizado este mes:** {info_m['nota']}/10 ({est_txt})"
                )
            ...
            if intentos_m:
                ...
                for p_b in banco_actual_m:
                  if isinstance(p_b, dict):
                    temas_totales_banco.add(
                        str(p_b.get("subindice", "General")).strip()
                    )
              ...
              if resp_ult:
                ...
                ult_resumen["% Aciertos"] = (
                    ult_resumen["Aciertos"] / ult_resumen["Total"] * 100
                ).round(2)
                ...
                st.line_chart(
                    ult_resumen.set_index("subindice")["% Aciertos"],
                    use_container_width=True,
                )
            ...
            try:
              ...
              for mod_sel in modelos_cargador_sel:
                ...
                try:
                  ...
                  if "```json" in txt_json:
                    txt_json = (
                        txt_json.split("```json")[1].split("```")[0].strip()
                    )
                  elif "```" in txt_json:
              ...
              status_box.write(
                  f"💾 Registrando examen con {len(preguntas_unificadas)}"
      ...
      with tab_mis_resultados:
        ...
        if mis_intentos:
          for i in mis_intentos:
            fecha_str = (
                i["fecha_inicio"][:10] if i.get("fecha_inicio") else "N/A"
            )
            ...
            with st.expander(
                f"Examen #{i['id']} - {i.get('apartado')} | {fecha_str} | Nota:"
              ...
              if respuestas:
                ...
                st.dataframe(
                    resumen_cat.rename(
                        columns={"subindice": "Categoría / Tema"}
                    ),
                    use_container_width=True,
                    hide_index=True,
                )
              ...
              if erroneas:
                ...
                for idx_e, err in enumerate(erroneas, 1):
                  ...
                  st.markdown(
                      "- **Respuesta correcta:**"
                      f" `{err.get('respuesta_correcta_texto', 'No disponible')}`"
                  )
                  st.write("---")
              else:
                st.success(
    ...
    if not st.session_state.es_croma and tab_emp_trimestral:
      with tab_emp_trimestral:
        ...
        emps = (
            supabase.table("empleados").select("id, nombre").execute().data
        )
          ...
          if intentos_filtrados:
            ...
            def format_func(it_id):
              ...
              est_it = obtener_estado_evaluacion(
                  it.get("porcentaje_obtenido", 0), it.get("sobrepasado_tiempo")
              )
              return (
                  f"ID #{it['id']} - {it.get('nombre_empleado')}"
                  f" ({it.get('apartado')}) | Nota: {it.get('nota', 0)}/10"
                  f" [{est_it}]"
              )
            ...
            respuestas_lista = json.loads(
                json.dumps(intento_obj.get("respuestas_usuario", []))
            )
            ...
            if respuestas_lista:
              ...
              st.info(
                  ...
                  f" **{p_objetivo.get('opcion_elegida')}** | Estado actual:"
                  f" **{'Correcta' if p_objetivo.get('es_correcta') else 'Incorrecta'}**"
              )
                ...
                if btn_guardar_edit:
                  if not persona_modifica.strip():
                    ...
                    try:
                      ...
                      nuevo_porc = round(
                          (correctas_nuevas / total_preg) * 100, 2
                      )
                      nueva_nota = round(
                          (correctas_nuevas / total_preg) * 10, 2
                      )
    ...
    if st.session_state.es_croma and tab_admin_export:
          ...
          if intentos_exp_filtrados:
            ...
            for i in intentos_exp_filtrados:
              est_exp = obtener_estado_evaluacion(
                  i.get("porcentaje_obtenido", 0), i.get("sobrepasado_tiempo")
              )
              opciones_examenes.append(
                  f"Examen #{i['id']} - {i.get('nombre_empleado')} |"
            ...
            with col_btn2:
              ...
              if pdf_bytes_ia:
                st.download_button(
                    ...
                    file_name=(
                        "Analisis_IA_"
                        f"{emp_exp_sel_nom.replace(' ', '_')}_{anio_exp_ia}.pdf"
                    ),
                    mime="application/pdf",
                    use_container_width=True,
      ...
      if archivo_csv_import is not None:
        if st.button(
            "🚀 Procesar e Importar CSV a la Base de Datos",
            use_container_width=True,
        ):
          try:
        ...
        map_empleados = {
            emp["nombre"].strip().lower(): emp["id"]
            for emp in (res_emp_all.data or [])
        }
      ...
      for idx_row, row in df_csv.iterrows():
        nombre_emp = str(
            row.get("nombre empleado") or row.get("nombre_empleado") or ""
        ).strip()
