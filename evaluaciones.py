...
# ---------------------------------------------------------
    ...
    # MENÚ PRINCIPAL
        ...
        if st.session_state.es_croma:
            tab_examenes, tab_eval_trimestral, tab_admin_manual, tab_admin_resultados, tab_admin_export, tab_admin_analisis, tab_admin_informes_ia, tab_admin_gestion = st.tabs([
                "📝 Realizar Examen",
            ...
            tab_examenes, tab_eval_trimestral, tab_mis_resultados, tab_mi_analisis = st.tabs([
                "📝 Realizar Examen", 
                "📅 Evaluación Trimestral",
        ...
        # TAB: EVALUACIÓN TRIMESTRAL (ADMIN Y EMPLEADO)
        with tab_eval_trimestral:
            st.subheader("📅 Gestión de Evaluación Trimestral")
            
            if st.session_state.es_croma:
                # SUBMENÚ PARA ADMINISTRADOR
                sub_tab_admin_eval, sub_tab_admin_config = st.tabs([
                    "📊 Evaluación Trimestral Administrador",
                    "⚙️ Configuración Trimestral"
                ])
                
                with sub_tab_admin_eval:
                    st.markdown("### 📋 Panel de Evaluación Trimestral (Administrador)")
                    st.caption("Módulo de administración para la revisión, asignación y seguimiento de evaluaciones trimestrales.")
                    
                    # Carga de datos e informes trimestrales
                    col_t1, col_t2 = st.columns(2)
                    with col_t1:
                        trimestre_sel = st.selectbox("Seleccionar Trimestre:", ["Q1", "Q2", "Q3", "Q4"], key="admin_trimestre_sel")
                    with col_t2:
                        anio_trim_sel = st.number_input("Año:", min_value=2020, max_value=2030, value=datetime.datetime.now().year, key="admin_anio_trim_sel")

                    st.markdown("---")
                    st.markdown("#### 👥 Resumen Global de Rendimiento Trimestral")
                    try:
                        res_eval_trim = supabase.table("intentos_examen").select("*").eq("activo", True).execute()
                        df_trim = pd.DataFrame(res_eval_trim.data) if res_eval_trim.data else pd.DataFrame()
                        
                        if not df_trim.empty and "fecha_inicio" in df_trim.columns:
                            df_trim["fecha_inicio"] = pd.to_datetime(df_trim["fecha_inicio"])
                            df_trim["trimestre"] = "Q" + df_trim["fecha_inicio"].dt.quarter.astype(str)
                            df_trim["anio"] = df_trim["fecha_inicio"].dt.year
                            
                            df_filtrado_trim = df_trim[(df_trim["trimestre"] == trimestre_sel) & (df_trim["anio"] == anio_trim_sel)]
                            
                            if not df_filtrado_trim.empty:
                                st.dataframe(
                                    df_filtrado_trim[["id", "nombre_empleado", "apartado", "nota", "porcentaje_obtenido", "fecha_inicio"]],
                                    use_container_width=True,
                                    hide_index=True
                                )
                                st.bar_chart(df_filtrado_trim.set_index("nombre_empleado")["nota"])
                            else:
                                st.info(f"No hay registros de evaluación para {trimestre_sel} {anio_trim_sel}.")
                        else:
                            st.info("No se encontraron datos registrados para procesar la evaluación trimestral.")
                    except Exception as e_trim:
                        st.error(f"Error al cargar evaluaciones trimestrales: {e_trim}")

                with sub_tab_admin_config:
                    st.markdown("### ⚙️ Ajustes de Evaluación Trimestral")
                    st.text_input("Nombre del periodo de evaluación trimestral activa:", value=f"Evaluación Trimestral - {trimestre_sel} {anio_trim_sel}")
                    st.number_input("Puntuación mínima aprobatoria trimestral (%):", min_value=0.0, max_value=100.0, value=70.0)
                    if st.button("Guardar Configuración Trimestral", use_container_width=True):
                        st.success("Configuración trimestral actualizada correctamente.")

            else:
                # SUBMENÚ PARA EMPLEADO
                sub_tab_emp_eval, sub_tab_emp_hist = st.tabs([
                    "📝 Mi Evaluación Trimestral",
                    "📜 Histórico Trimestral"
                ])

                with sub_tab_emp_eval:
                    st.markdown("### 📝 Mi Evaluación Trimestral Actual")
                    st.info("Espacio destinado a la realización y consulta del estado de tu evaluación trimestral activa.")
                    
                    try:
                        res_emp_trim = supabase.table("intentos_examen").select("*")\
                            .eq("empleado_id", st.session_state.user_id)\
                            .eq("activo", True).execute()
                        
                        df_emp_trim = pd.DataFrame(res_emp_trim.data) if res_emp_trim.data else pd.DataFrame()
                        
                        if not df_emp_trim.empty and "fecha_inicio" in df_emp_trim.columns:
                            df_emp_trim["fecha_inicio"] = pd.to_datetime(df_emp_trim["fecha_inicio"])
                            df_emp_trim["trimestre"] = "Q" + df_emp_trim["fecha_inicio"].dt.quarter.astype(str)
                            df_emp_trim["anio"] = df_emp_trim["fecha_inicio"].dt.year
                            
                            st.markdown("#### 📊 Mis Notas del Trimestre Actual")
                            st.dataframe(
                                df_emp_trim[["apartado", "nota", "porcentaje_obtenido", "trimestre", "anio"]],
                                use_container_width=True,
                                hide_index=True
                            )
                        else:
                            st.write("No registras evaluaciones dentro del periodo trimestral vigente.")
                    except Exception as e_emp_t:
                        st.error(f"Error al obtener tus datos trimestrales: {e_emp_t}")

                with sub_tab_emp_hist:
                    st.markdown("### 📜 Histórico de Evaluaciones Trimestrales")
                    st.caption("Consulta tus resultados trimestrales consolidados de periodos anteriores.")
            ...
            with tab_admin_informes_ia:
                ...
                try:
                    ...
                    if ia_informes_data:
                        ...
                        st.info("No se encontraron informes generados.")
                except Exception as e_ia_mng:
                    st.error(f"Error al cargar la gestión de informes IA: {e_ia_mng}")
